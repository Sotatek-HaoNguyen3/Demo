...
export default class CircularProgress extends React.PureComponent<IProgressCircleProps> {
    static defaultProps = {
        tintColor: 'black',
    ...
    render() {
        ...
        const strokeDasharrayTint =
            dashedTint['gap'] > 0 ? Object.values(dashedTint).map((value) => parseInt(value)) : null;
        ...
        return (
            <View style={style}>
                ...
                {children && <View style={styles.localChildrenContainerStyle}>{children(fill)}</View>}
            </View>
        );
    }
}
