...
import { Camera, useCameraDevices, useFrameProcessor } from 'react-native-vision-camera/src';
...
export default function BarcodeScanner({ route, navigation }) {
    ...
    const continuous = route.params ? route.params.continuous : true;
    ...
    const format = React.useMemo(() => {
        ...
        if (selectedCam) {
            for (let index = 0; index < selectedCam.formats.length; index++) {
                const formated = selectedCam.formats[index];
        ...
        <SafeAreaView style={styles.container}>
            ...
            <ActionSheet
                ...
                onPress={async (index) => {
                    if (pressedResult) {
                        if (index === 0) {
                            navigation.navigate('Infor', { barcode: pressedResult });
                        } else if (index === 1) {
            ...
            />
            <Svg style={StyleSheet.absoluteFill} preserveAspectRatio="xMidYMid slice" viewBox={getViewBox()}>
                {regionEnabled && (
                    <Rect
